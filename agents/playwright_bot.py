...
BotEventType: TypeAlias = Literal["bot_spawn", "bot_move", "bot_click", "bot_log", "bot_complete"]
...
def _position_for_page(page: str) -> tuple[float, float, float]:
    ...
    return (round(radius * __import__("math").cos(angle), 2), round(0.7 + random.random() * 1.8, 2), round(radius * __import__("math").sin(angle), 2))
